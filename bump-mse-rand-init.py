...
N_init = 10
params_matrix = np.zeros([N_params, N_point])
a0 = np.random.rand(N_init) * 2
mu0 = np.random.rand(N_init)
log_var0 = np.random.rand(N_init)
b0 = np.random.rand(N_init) * (-3)

for k in range(N_init):
    p_init = list()
    p_init.append(a0[k])
    p_init.append(mu0[k])
    p_init.append(log_var0[k])
    p_init.append(b0[k])
    params_matrix = modelfit.init_p(params_matrix, p_init, 0, n3)

    err = 1000

    print('{}th initialization started'.format(str(k+1)))

    for n in range(30):
        c = 0
        for i in range(n3):
            est_matrix[:, c] = modelfit.f_v_a(ang_vector, params_matrix[1:3, c], params_matrix[0, c])
            b_est = np.mean(V_matrix[:, c] - est_matrix[:, c])
            params_matrix[-1, c] = b_est
            a_est = modelfit.opt_a(ang_vector, V_matrix[:, c], params_matrix[1, c], params_matrix[2, c], b_est)
            params_matrix[0, c] = a_est
            est = minimize(modelfit.mse_fv_baseline_a, params_matrix[1:3, c], args=(ang_vector, V_matrix[:, c], a_est, b_est))
            p_est = est.x
            params_matrix[1:3, c] = p_est
            c += 1
        print('-- {}th estimation finished, the last a, mean, log var, b estimates are  {}'.format(str(n+1), params_matrix[:, c-1]))

    est = modelfit.update_p(est_matrix, params_matrix, ang_vector, 0, n3)
    mse = modelfit.mse(V_matrix, est)

    if mse < err:
        p_matrix = params_matrix
        err = mse
